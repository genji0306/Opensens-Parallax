...
class IdeaGenerator:
    ...
    def generate_ideas(
        ...
        for i in range(num_ideas):
            logger.info("Generating idea %d/%d", i + 1, num_ideas)
            try:
                ...
                if idea_dict:
                    idea_archive.append(idea_dict["raw"])
                    ideas.append(idea_dict["idea"])
        ...
        for idea in ideas:
            try:
                novelty_result = self.check_novelty(idea, existing_papers, llm)
    ...
    def _generate_single_idea(
        ...
        response = llm.chat(messages=messages, temperature=0.7, max_tokens=2000)
        json_output = self._parse_json(response)
        if not json_output:
            logger.warning("Failed to parse JSON from initial idea generation")
            return None
