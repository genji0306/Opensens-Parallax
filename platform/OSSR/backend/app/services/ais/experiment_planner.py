...
class ExperimentPlanner:
    ...
    TEMPLATES_DIR = Path(__file__).resolve().parents[5] / "tools" / "ai-scientist" / "templates"
    ...
    def plan_experiment(
        ...
        Steps:
        1. Select best-matching template based on idea keywords vs TEMPLATE_DOMAINS.
        2. Generate seed_ideas.json from the idea's hypothesis + methodology.
        3. Configure experiment params (max_runs, model, budget).
        4. Save to DB and return spec.
        ...
        """
        logger.info("[ExperimentPlanner] Planning experiment for idea %s", idea.idea_id)
        ...
        logger.info(
            "[ExperimentPlanner] Spec %s created — template=%s, seed_ideas=%d",
            spec.spec_id, template, len(seed_ideas),
        )
        return spec
    ...
    def list_specs(self, run_id: Optional[str] = None) -> List[ExperimentSpec]:
        ...
        return [
            ExperimentSpec(
                ...
                idea_id=r["idea_id"],
                template=r["template"],
                seed_ideas=json.loads(r["seed_ideas"]) if r["seed_ideas"] else [],
