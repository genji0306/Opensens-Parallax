...
class ExperimentRunner:
    ...
    AI_SCIENTIST_DIR = Path(__file__).resolve().parents[5] / "tools" / "ai-scientist"
    ...
    def _run_ai_scientist(
        ...
        cmd = [
            "python", str(self.AI_SCIENTIST_DIR / "launch_scientist.py"),
            "--experiment", str(template_dir),
            "--model", model,
            "--num-ideas", str(max_runs),
            "--seed-ideas", str(seed_path),
        ]
        if skip_writeup:
        ...
        env = {**os.environ, "PYTHONPATH": str(self.AI_SCIENTIST_DIR)}
    ...
    def _write_seed_ideas(self, spec: ExperimentSpec, work_dir: Path):
        """Write seed_ideas.json to the work directory."""
        seed_path = work_dir / "seed_ideas.json"
    ...
    def _row_to_result(self, row) -> ExperimentResult:
        """Convert a sqlite3.Row to an ExperimentResult dataclass."""
        return ExperimentResult(
