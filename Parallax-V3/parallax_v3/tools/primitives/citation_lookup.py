...
class CitationLookup(TypedTool):
    def __init__(self, workspace_path: Path, session_id: str | None = None, base_url: str | None = None):
        ...
        workspace = (root / session_id) if session_id else root
        workspace = workspace.resolve()
        workspace.mkdir(parents=True, exist_ok=True)
        object.__setattr__(self, "workspace_path", workspace)
        object.__setattr__(self, "session_id", session_id)
        object.__setattr__(self, "base_url", base_url or "https://api.semanticscholar.org/graph/v1")
        object.__setattr__(self, "_cache_path", self.workspace_path / "citations" / "s2_cache.json")
    ...
    def search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        self._cache.setdefault("pool", []).extend(deduped)
        self._save_cache()
        return deduped
