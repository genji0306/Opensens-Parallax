...
def connect_disconnect_usb():
    ...
    if dev is None:
        ...
        try:
            ...
            get_calibration()
            set_cell_status(dev, main_window, False)  # Cell off
            set_control_mode(dev, main_window, False)  # Potentiostatic control
...
def dac_calibrate():
    ...
    send_command(dev, main_window, b'DACCAL', b'OK',
                 "DAC calibration performed.")
    get_dac_calibration(dev, main_window)
...
def set_calibration():
    ...
    set_dac_calibration(dev, main_window)
    set_offset(dev, main_window, current_offset, potential_offset)
    set_shunt_calibration(dev, main_window, shunt_calibration)
...
def get_calibration():
    ...
    get_dac_calibration(dev, main_window)
    get_offset()
    get_shunt_calibration(dev, main_window, shunt_calibration)
...
def set_current_range():
    ...
    if send_command(dev, main_window, commandstring, b'OK'):
...
def set_output(value_units_index, value):
    ...
    if value_units_index == 0:
        send_command(dev, main_window, b'DACSET '+decimal_to_dac_bytes(value/8.*2. **
                                                                       19+int(round(potential_offset/4.))), b'OK')
    elif value_units_index == 1:
        send_command(dev, main_window, b'DACSET '+decimal_to_dac_bytes(value/(25. /
                                                                              (shunt_calibration[currentrange]*100.**currentrange))*2.**19+int(round(current_offset/4.))), b'OK')
    elif value_units_index == 2:
        send_command(dev, main_window, b'DACSET ' +
...
def cd_start(cd_parameters):
    ...
    if check_state(state, [States.Idle, States.Stationary_Graph, States.Measuring_start]):
        ...
        set_control_mode(dev, main_window, True)  # Galvanostatic control
        ...
        set_cell_status(dev, main_window, True)  # Cell on
...
def cd_stop(interrupted=True):
    ...
    global state
    if check_state(state, [States.Measuring_CD]):
...
def cv_start(cv_parameters):
    ...
    if check_state(state, [States.Idle, States.Stationary_Graph, States.Measuring_start]):
        ...
        set_control_mode(dev, main_window, False)
        set_current_range()
        ...
        set_cell_status(dev, main_window, True)
...
def cv_stop(interrupted=True):
    ...
    global state
    if check_state(state, [States.Measuring_CV]):
        cv_outputfile.close()
...
def rate_stop(interrupted=True):
    ...
    global state
    if check_state(state, [States.Measuring_Rate]):
        # Keep displaying the last plot until the user clicks a button
        set_cell_status(dev, main_window, False)
...
class manual(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.cell_connect_on.clicked.connect(
            lambda: set_cell_status(dev, main_window, True))
        self.cell_connect_off.clicked.connect(
            lambda: set_cell_status(dev, main_window, False))
        self.potentiostat.clicked.connect(
            lambda: set_control_mode(dev, main_window, False))
        self.galvanostatic.clicked.connect(
            lambda: set_control_mode(dev, main_window, True))
