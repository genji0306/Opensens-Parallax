...
export class OssrDebateAdapter {
  ...
  async getDebateFrame(simulationId: string): Promise<DebateFrame | null> {
    try {
      const resp = await this.get<DebateFrame>(`/simulate/${simulationId}/frame`);
  ...
  async getGraphSnapshot(simulationId: string, round?: number): Promise<GraphSnapshot | null> {
    try {
      const params = round !== undefined ? `?round=${round}&format=d3` : "?format=d3";
      const resp = await this.get<GraphSnapshot>(`/simulate/${simulationId}/graph${params}`);
      return resp.data ?? null;
    } catch {
      return null;
    }
  ...
  async getScoreboard(simulationId: string, round?: number): Promise<MirofishScoreboard | null> {
    try {
      const params = round !== undefined ? `?round=${round}` : "";
      const resp = await this.get<MirofishScoreboard>(`/simulate/${simulationId}/scoreboard${params}`);
      return resp.data ?? null;
    } catch {
      return null;
    }
  ...
  async getAnalystFeed(simulationId: string): Promise<AnalystFeedEntry[]> {
    try {
      const resp = await this.get<AnalystFeedEntry[]>(`/simulate/${simulationId}/analyst-feed`);
      return resp.data ?? [];
    } catch {
      return [];
    }
  ...
  async getStances(simulationId: string): Promise<AgentStance[]> {
    try {
      const resp = await this.get<AgentStance[]>(`/simulate/${simulationId}/stances`);
      return resp.data ?? [];
    } catch {
      return [];
    }
  ...
  /**
   * Subscribe to SSE stream with Mirofish event support.
   * Extends the base subscribeToStream with orchestrator event callbacks.
   */
  subscribeToMirofishStream(
    simulationId: string,
    handlers: {
      onTurn: (turn: DiscussionTurn) => void;
      onComplete?: () => void;
      onPaused?: () => void;
      onResumed?: () => void;
      onFrame?: (frame: DebateFrame) => void;
      onScoreboard?: (data: { round_num: number; scoreboard: MirofishScoreboard }) => void;
      onGraphUpdate?: (data: { round_num: number; snapshot: GraphSnapshot }) => void;
      onAnalystNote?: (entry: AnalystFeedEntry) => void;
      onStanceUpdate?: (data: { agent_id: string; round_num: number; stances: AgentStance[] }) => void;
    },
  ): () => void {
    const url = `${this.baseUrl}/api/research/simulate/${simulationId}/stream`;
    const es = new EventSource(url);
    ...
    es.addEventListener("turn", (event: MessageEvent) => {
      try {
        handlers.onTurn(JSON.parse(event.data));
      } catch { /* ignore */ }
    });

    es.addEventListener("completed", () => { handlers.onComplete?.(); es.close(); });
    es.addEventListener("failed", () => { handlers.onComplete?.(); es.close(); });
    es.addEventListener("paused", () => { handlers.onPaused?.(); });
    es.addEventListener("resumed", () => { handlers.onResumed?.(); });

    // Mirofish orchestrator events
    es.addEventListener("frame", (event: MessageEvent) => {
      try { handlers.onFrame?.(JSON.parse(event.data)); } catch { /* ignore */ }
    });

    es.addEventListener("scoreboard", (event: MessageEvent) => {
      try { handlers.onScoreboard?.(JSON.parse(event.data)); } catch { /* ignore */ }
    });

    es.addEventListener("graph_update", (event: MessageEvent) => {
      try { handlers.onGraphUpdate?.(JSON.parse(event.data)); } catch { /* ignore */ }
    });

    es.addEventListener("analyst_note", (event: MessageEvent) => {
      try { handlers.onAnalystNote?.(JSON.parse(event.data)); } catch { /* ignore */ }
    });

    es.addEventListener("stance_update", (event: MessageEvent) => {
      try { handlers.onStanceUpdate?.(JSON.parse(event.data)); } catch { /* ignore */ }
    });

    es.onerror = () => {
      if (es.readyState === EventSource.CLOSED) handlers.onComplete?.();
    };
