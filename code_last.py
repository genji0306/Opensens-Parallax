...
def format_box_for_display(box):
    ...
    # Make background 10% darker to make the border color
    r, g, b = int(0.9*color.red()), int(0.9 *
                                        color.green()), int(0.9*color.blue())
    # Apply the border
    box.setStyleSheet(
        "QGroupBox { border: 1px solid rgb(%d,%d,%d); border-radius: 4px; margin-top: 0.5em; font-weight: normal; color: gray;} QGroupBox::title {subcontrol-origin: margin; left: 10px; padding: 0 3px 0 3px;}" % (r, g, b))


def format_box_for_parameter(box):
    """Adjust the appearance of a groupbox border for parameter input."""
    color = box.palette().color(QtGui.QPalette.Background)  # Get the background color
    # Make background 30% darker to make the border color
    r, g, b = int(0.7*color.red()), int(0.7 *
                                        color.green()), int(0.7*color.blue())
    # Apply the border
    box.setStyleSheet(
        "QGroupBox { border: 1px solid rgb(%d,%d,%d); border-radius: 4px; margin-top: 0.5em; font-weight: bold} QGroupBox::title {subcontrol-origin: margin; left: 10px; padding: 0 3px 0 3px;}" % (r, g, b))
    ...
    if state not in desired_states:
        if state == 0:
            ...
            QtGui.QMessageBox.critical(
                mainwidget, "Error", "This command cannot be executed in the current state.")
        return False
    else:
        return True
...
def send_command(command_string, expected_response, log_msg=None):
    ...
    if dev is not None:  # Make sure it's connected
        ...
        if response != expected_response:
            QtGui.QMessageBox.critical(mainwidget, "Unexpected Response", "The command \"%s\" resulted in an unexpected response. The expected response was \"%s\"; the actual response was \"%s\"" % (
...
def auto_current_range():
    ...
    if relativecurrent > 1.05 and currentrange != 0 and cv_range_checkboxes[currentrange-1].isChecked():
        overcounter += 1
    else:
        overcounter = 0
    # Switch to lower current range (if possible) after three detections
    if relativecurrent < 0.0095 and currentrange != 2 and cv_range_checkboxes[currentrange+1].isChecked():
        undercounter += 1
    else:
        undercounter = 0
    if overcounter > 3:
...
def get_next_enabled_current_range(desired_currentrange):
    ...
    # Look for an enabled current range, going up in current range
    for i in range(desired_currentrange, -1, -1):
        if cv_range_checkboxes[i].isChecked():
            found_currentrange = i
            range_found = True
            break
    if not range_found:
        # Look for an enabled current range, going down in current range
        for i in range(desired_currentrange, 3):
            if cv_range_checkboxes[i].isChecked():
                found_currentrange = i
                break
...
def get_offset():
    ...
    if dev is not None:  # Make sure it's connected
        ...
        if response != bytes([255, 255, 255, 255, 255, 255]):
            ...
            hardware_calibration_potential_offset.setText(
                "%d" % potential_offset)
    ...
    if dev is not None:  # Make sure it's connected
        ...
        if response != bytes([255, 255, 255, 255, 255, 255]):
            for i in range(0, 3):
                ...
                hardware_calibration_shuntvalues[i].setText(
                    "%.4f" % shunt_calibration[i])
            log_message("Shunt calibration values read from flash memory.")
        else:
            log_message(
                "No shunt calibration values were found in flash memory.")
    else:
        not_connected_errormessage()
    ...
    for i in range(0, 3):
        try:
            shunt_calibration[i] = float(
                hardware_calibration_shuntvalues[i].text())
...
def get_dac_calibration():
    ...
    if dev is not None:  # Make sure it's connected
        ...
        if response != bytes([255, 255, 255, 255, 255, 255]):
            ...
            log_message(
                "No DAC calibration values were found in flash memory.")
    else:
        not_connected_errormessage()
...
def set_output_from_gui():
    ...
    if value_units_index == 0:  # Potential (V)
        try:
            ...
            QtGui.QMessageBox.critical(
                mainwidget, "Not a number", "The value you have entered is not a floating-point number.")
            return
        ...
        try:
            ...
            QtGui.QMessageBox.critical(
                mainwidget, "Not a number", "The value you have entered is not an integer number.")
            return
    else:
        return
...
def choose_file(file_entry_field, questionstring):
    ...
    tuple_file = filedialog.getSaveFileName(
        mainwidget, questionstring, "", "ASCII data (*.txt)", options=QtGui.QFileDialog.DontConfirmOverwrite)
...
def cv_getparams():
    ...
    try:
        ...
        cv_parameters['scanrate'] = float(
            cv_scanrate_entry.text())/1e3  # Convert to V/s
        ...
        QtGui.QMessageBox.critical(
            mainwidget, "Not a number", "One or more parameters could not be interpreted as a number.")
        return False
...
def cv_validate_parameters():
    ...
    if cv_parameters['ubound'] < cv_parameters['lbound']:
        QtGui.QMessageBox.critical(
            mainwidget, "CV error", "The upper bound cannot be lower than the lower bound.")
        return False
    if cv_parameters['scanrate'] == 0:
        QtGui.QMessageBox.critical(
            mainwidget, "CV error", "The scan rate cannot be zero.")
        return False
    if (cv_parameters['scanrate'] > 0) and (cv_parameters['ubound'] < cv_parameters['startpot']):
        QtGui.QMessageBox.critical(
            mainwidget, "CV error", "For a positive scan rate, the start potential must be lower than the upper bound.")
        return False
    if (cv_parameters['scanrate'] < 0) and (cv_parameters['lbound'] > cv_parameters['startpot']):
        QtGui.QMessageBox.critical(
            mainwidget, "CV error", "For a negative scan rate, the start potential must be higher than the lower bound.")
        return False
    if cv_parameters['numsamples'] < 1:
        QtGui.QMessageBox.critical(
            mainwidget, "CV error", "The number of samples to average must be at least 1.")
        return False
    return True
...
def validate_file(filename):
    ...
    try:
        ...
        QtGui.QMessageBox.critical(
            mainwidget, "File error", "The specified output file path is not valid.")
        return False
...
def cv_scanrate_changed_callback():
    ...
    try:
        ...
        # Aims for approx. one (averaged) measurement every 2 to 4 mV for scan rates up to 20 mV/s
...
def cv_start():
    ...
    if check_state([States.Idle, States.Stationary_Graph]) and cv_getparams() and cv_validate_parameters() and validate_file(cv_parameters['filename']):
        ...
        hardware_manual_control_range_dropdown.setCurrentIndex(
            0)  # Start at highest current range
        set_current_range()
        ...
        read_potential_current()
        hardware_manual_control_range_dropdown.setCurrentIndex(get_next_enabled_current_range(
            current_range_from_current(current)))  # Another autorange, just to be sure
        set_current_range()
        ...
        log_message("CV measurement started. Saving to: %s" %
...
def cv_preview():
    ...
    if check_state([States.Idle, States.Stationary_Graph]) and cv_getparams() and cv_validate_parameters():
        ...
        # Automatic timestep calculation, resulting in 100 potential steps between lower and upper bound
        timestep = abs(
            (cv_parameters['ubound']-cv_parameters['lbound'])/100./cv_parameters['scanrate'])
        ...
        preview_cancel_button.show()
        # Keep displaying the CV preview until the user clicks a button
        state = States.Stationary_Graph
...
def cd_getparams():
    ...
    try:
        ...
        cd_parameters['chargecurrent'] = float(
            cd_chargecurrent_entry.text())/1e3  # convert uA to mA
        cd_parameters['dischargecurrent'] = float(
            cd_dischargecurrent_entry.text())/1e3  # convert uA to mA
...
def cd_validate_parameters():
    ...
    if cd_parameters['chargecurrent'] == 0.:
        QtGui.QMessageBox.critical(
            mainwidget, "Charge/discharge error", "The charge current cannot be zero.")
        return False
    if cd_parameters['dischargecurrent'] == 0.:
        QtGui.QMessageBox.critical(
            mainwidget, "Charge/discharge error", "The discharge current cannot be zero.")
        return False
...
def cd_start():
    ...
    if check_state([States.Idle, States.Stationary_Graph]) and cd_getparams() and cd_validate_parameters() and validate_file(cd_parameters['filename']):
        ...
        cd_charges = []
        cd_plot_curves = []
        # This file will contain time, potential, and current data
        ...
        # This file will contain capacity data for each cycle
        ...
        plot_frame.setLabel('left', 'Potential', units="V")
        # Draw potential as a function of charge in yellow
        cd_plot_curves.append(plot_frame.plot(pen='y'))
        log_message("Charge/discharge measurement started. Saving to: %s" %
                    cd_parameters['filename'])
        # Indicate the current cycle number
...
def cd_update():
    ...
    if cd_currentcycle > cd_parameters['numcycles']:
        ...
        if (cd_currentsetpoint > 0 and potential > cd_parameters['ubound']) or (cd_currentsetpoint < 0 and potential < cd_parameters['lbound']):
            ...
            cd_current_cycle_entry.setText(
                "%d" % cd_currentcycle)  # Indicate next cycle
...
def cd_stop(interrupted=True):
    ...
    if check_state([States.Measuring_CD]):
        ...
        if interrupted:
            log_message("Charge/discharge measurement interrupted. Calculated charges (in uAh): [" + ', '.join("%.2f" % (
                value*1e6) for value in cd_charges) + "]")  # Print list of inserted/extracted charges to the message log
        else:
            log_message("Charge/discharge measurement finished. Calculated charges (in uAh): [" + ', '.join("%.2f" % (
                value*1e6) for value in cd_charges) + "]")  # Print list of inserted/extracted charges to the message log
...
def rate_getparams():
    ...
    try:
        ...
        rate_parameters['one_c_current'] = float(
            rate_capacity_entry.text())/1e3  # Convert uA to mA
        rate_parameters['crates'] = [
            float(x) for x in rate_crates_entry.text().split(",")]
        rate_parameters['currents'] = [
            rate_parameters['one_c_current']*rc for rc in rate_parameters['crates']]
...
def rate_validate_parameters():
    ...
    if 0. in rate_parameters['currents']:
        QtGui.QMessageBox.critical(
            mainwidget, "Rate testing error", "The charge/discharge current cannot be zero.")
        return False
...
def rate_start():
    ...
    if check_state([States.Idle, States.Stationary_Graph]) and rate_getparams() and rate_validate_parameters() and validate_file(rate_parameters['filename']):
        ...
        hardware_manual_control_range_dropdown.setCurrentIndex(current_range_from_current(
            rate_current))  # Determine the proper current range for the current setpoint
        set_current_range()  # Set new current range
        ...
        log_message("Rate testing started. Saving to: %s" %
                    rate_parameters['filename'])
        rate_current_crate_entry.setText(
            "%d" % rate_parameters['crates'][crate_index])  # Indicate the current C-rate
...
def rate_update():
    ...
    if (rate_halfcycle_countdown % 2 == 0 and potential > rate_parameters['ubound']) or (rate_halfcycle_countdown % 2 != 0 and potential < rate_parameters['lbound']):
        ...
        for data in [rate_time_data, rate_potential_data, rate_current_data]:
            data.clear()
        rate_current_crate_entry.setText(
            "%d" % rate_parameters['crates'][crate_index])  # Indicate the next C-rate
...
potential_monitor, potential_monitor_box = make_groupbox_indicator(
    "Measured potential", "+#.### V")
potential_monitor.setFont(QtGui.QFont("monospace", 32))
current_monitor, current_monitor_box = make_groupbox_indicator(
    "Measured current", "+#.### mA")
...
cell_status_monitor, cell_status_monitor_box = make_groupbox_indicator(
    "Cell status", "        ")
cell_status_monitor.setFont(custom_size_font(14))
control_mode_monitor, control_mode_monitor_box = make_groupbox_indicator(
    "Control mode", "             ")
control_mode_monitor.setFont(custom_size_font(14))
current_range_monitor, current_range_monitor_box = make_groupbox_indicator(
    "Current range", "     ")
...
hardware_device_info_box = QtGui.QGroupBox(
    title="Device Information", flat=False)
...
hardware_device_info_text = QtGui.QLabel(
    "Manufacturer: \nProduct: \nSerial #: ")
...
hardware_calibration_dac_offset = make_label_entry(
    hardware_calibration_dac_vlayout, "DAC Offset")
hardware_calibration_dac_gain = make_label_entry(
    hardware_calibration_dac_vlayout, "DAC Gain")
hardware_calibration_dac_calibrate_button = QtGui.QPushButton(
    "Auto\nCalibrate")
...
hardware_calibration_dac_hlayout.addWidget(
    hardware_calibration_dac_calibrate_button)
...
hardware_calibration_offset_hlayout.addLayout(
    hardware_calibration_offset_vlayout)
hardware_calibration_potential_offset = make_label_entry(
    hardware_calibration_offset_vlayout, "Pot. Offset")
hardware_calibration_potential_offset.editingFinished.connect(
    offset_changed_callback)
hardware_calibration_potential_offset.setText("0")
hardware_calibration_current_offset = make_label_entry(
    hardware_calibration_offset_vlayout, "Curr. Offset")
hardware_calibration_current_offset.editingFinished.connect(
    offset_changed_callback)
...
hardware_calibration_offset_hlayout.addWidget(
    hardware_calibration_adc_measure_button)

hardware_calibration_shunt_resistor_layout = QtGui.QHBoxLayout()
hardware_calibration_box_layout.addLayout(
    hardware_calibration_shunt_resistor_layout)
hardware_calibration_shuntvalues = [make_label_entry(
    hardware_calibration_shunt_resistor_layout, "R%d" % i) for i in range(1, 4)]
for i in range(0, 3):
    hardware_calibration_shuntvalues[i].editingFinished.connect(
...
hardware_manual_control_box = QtGui.QGroupBox(
    title="Manual Control", flat=False)
...
hardware_manual_control_cell_on_button.clicked.connect(
    lambda: set_cell_status(True))
hardware_manual_control_cell_layout.addWidget(
    hardware_manual_control_cell_on_button)
hardware_manual_control_cell_off_button = QtGui.QPushButton("Off")
hardware_manual_control_cell_off_button.clicked.connect(
    lambda: set_cell_status(False))
hardware_manual_control_cell_layout.addWidget(
    hardware_manual_control_cell_off_button)
hardware_manual_control_box_layout.addLayout(
    hardware_manual_control_cell_layout)

hardware_manual_control_mode_layout = QtGui.QHBoxLayout()
hardware_manual_control_mode_layout.addWidget(QtGui.QLabel("Mode"))
hardware_manual_control_mode_potentiostat_button = QtGui.QPushButton(
    "Potentiostatic")
hardware_manual_control_mode_potentiostat_button.clicked.connect(
    lambda: set_control_mode(False))
hardware_manual_control_mode_layout.addWidget(
    hardware_manual_control_mode_potentiostat_button)
hardware_manual_control_mode_galvanostat_button = QtGui.QPushButton(
    "Galvanostatic")
hardware_manual_control_mode_galvanostat_button.clicked.connect(
    lambda: set_control_mode(True))
hardware_manual_control_mode_layout.addWidget(
    hardware_manual_control_mode_galvanostat_button)
hardware_manual_control_box_layout.addLayout(
    hardware_manual_control_mode_layout)
...
hardware_manual_control_range_layout.addWidget(
    hardware_manual_control_range_dropdown)
...
hardware_manual_control_box_layout.addLayout(
    hardware_manual_control_range_layout)
...
hardware_manual_control_output_layout.addWidget(
...
hardware_manual_control_output_layout.addWidget(
    hardware_manual_control_output_entry)
...
hardware_manual_control_output_layout.addWidget(
    hardware_manual_control_output_set_button)
hardware_manual_control_box_layout.addLayout(
    hardware_manual_control_output_layout)
...
hardware_log_choose_button.clicked.connect(lambda: choose_file(
    hardware_log_filename, "Choose where to save the log data"))
...
cv_params_box = QtGui.QGroupBox(
    title="Cyclic voltammetry parameters", flat=False)
...
cv_file_choose_button.clicked.connect(lambda: choose_file(
    cv_file_entry, "Choose where to save the CV measurement data"))
...
cd_params_box = QtGui.QGroupBox(
    title="Charge/discharge parameters", flat=False)
...
cd_chargecurrent_entry = make_label_entry(
    cd_params_layout, u"Charge current (µA)")
cd_dischargecurrent_entry = make_label_entry(
    cd_params_layout, u"Discharge current (µA)")
cd_numcycles_entry = make_label_entry(
    cd_params_layout, "Number of half cycles")
...
cd_file_choose_button.clicked.connect(lambda: choose_file(
    cd_file_entry, "Choose where to save the charge/discharge measurement data"))
...
rate_numcycles_entry = make_label_entry(
    rate_params_layout, u"Cycles per C-rate")
...
rate_file_choose_button.clicked.connect(lambda: choose_file(
    rate_file_entry, "Choose where to save the rate testing measurement data"))
...
# Calls periodic_update() every adcread_interval (as defined in the beginning of this program)
timer.start(qt_timer_period)

log_message(
    "Program started. Press the \"Connect\" button in the hardware tab to connect to the USB interface.")

win.show()  # Show the main window
# Keep the program running by periodically calling the periodic_update() until the GUI window is closed
sys.exit(app.exec_())
