...
def get_connection() -> sqlite3.Connection:
    ...
    if conn is None:
        ...
        conn.execute("PRAGMA foreign_keys=OFF")  # OFF: demo data has orphaned refs
        conn.execute("PRAGMA busy_timeout=30000")  # 30s: multiple threads write concurrently
...
def init_db():
    ...
    conn.executescript("""
        ...
        -- Run-scoped paper/topic associations (junction tables for data isolation)
        ...
        -- Paper Rehabilitation (uploaded paper review & revision pipeline)

        CREATE TABLE IF NOT EXISTS paper_uploads (
            upload_id TEXT PRIMARY KEY,
            title TEXT NOT NULL DEFAULT '',
            language TEXT NOT NULL DEFAULT 'en',
            detected_field TEXT NOT NULL DEFAULT '',
            sections TEXT NOT NULL DEFAULT '[]',
            raw_references TEXT NOT NULL DEFAULT '[]',
            full_text TEXT NOT NULL DEFAULT '',
            metadata TEXT NOT NULL DEFAULT '{}',
            source_filename TEXT NOT NULL DEFAULT '',
            status TEXT NOT NULL DEFAULT 'uploaded',
            review_config TEXT NOT NULL DEFAULT '{}',
            review_rounds TEXT NOT NULL DEFAULT '[]',
            source_audit TEXT NOT NULL DEFAULT '{}',
            reviewers TEXT NOT NULL DEFAULT '[]',
            authors TEXT NOT NULL DEFAULT '[]',
            current_draft TEXT NOT NULL DEFAULT '',
            score_progression TEXT NOT NULL DEFAULT '[]',
            created_at TEXT NOT NULL DEFAULT '',
            updated_at TEXT NOT NULL DEFAULT '',
            error TEXT
        );

        CREATE INDEX IF NOT EXISTS idx_paper_uploads_status ON paper_uploads(status);
        ...
        -- ═══════════════════════════════════════════════════════════════
        -- V2 Workflow Graph Engine tables
        -- ═══════════════════════════════════════════════════════════════
        ...
        -- Workflow nodes: each step in the pipeline DAG
        ...
        CREATE INDEX IF NOT EXISTS idx_wf_nodes_run ON workflow_nodes(run_id);
        CREATE INDEX IF NOT EXISTS idx_wf_nodes_status ON workflow_nodes(status);
        CREATE INDEX IF NOT EXISTS idx_wf_nodes_type ON workflow_nodes(run_id, node_type);
        ...
        CREATE INDEX IF NOT EXISTS idx_wf_edges_run ON workflow_edges(run_id);
        ...
        CREATE INDEX IF NOT EXISTS idx_draft_versions_run ON draft_versions(run_id);
        ...
        CREATE TABLE IF NOT EXISTS project_templates (
            ...
            description TEXT NOT NULL DEFAULT '',
            config TEXT NOT NULL DEFAULT '{}',
            step_settings TEXT NOT NULL DEFAULT '{}',
...
def run_migrations():
    """Run any pending schema migrations. Called after init_db."""
    conn = get_connection()
    ...
    for version, description, sql in migrations:
        ...
        try:
            ...
            conn.execute(
                ...
                (version, description, __import__("datetime").datetime.now().isoformat()),
            )
            conn.commit()
            applied += 1
        except Exception as e:
            import logging
            logging.getLogger(__name__).error("Migration %d failed: %s", version, e)
            break

    if applied:
        import logging
        logging.getLogger(__name__).info("Applied %d schema migrations (now at v%d)",
                                         applied, current_version + applied)


def _get_migrations():
    """Return list of (version, description, sql) tuples for incremental schema changes."""
    return [
        (1, "V2 workflow graph engine — initial tables", """
            -- Already created by init_db (CREATE IF NOT EXISTS), so this is a no-op marker.
            -- Future migrations add ALTER TABLE, new columns, etc.
            SELECT 1;
        """),
        (2, "Knowledge artifacts table (P-2)", """
            CREATE TABLE IF NOT EXISTS knowledge_artifacts (
                artifact_id TEXT PRIMARY KEY,
                run_id TEXT NOT NULL,
                data TEXT NOT NULL DEFAULT '{}',
            ...
            CREATE INDEX IF NOT EXISTS idx_knowledge_artifacts_run ON knowledge_artifacts(run_id);
        """),
        (3, "Draft version history + project templates", """
            CREATE TABLE IF NOT EXISTS draft_versions (
                version_id TEXT PRIMARY KEY,
                draft_id TEXT NOT NULL,
                run_id TEXT NOT NULL DEFAULT '',
