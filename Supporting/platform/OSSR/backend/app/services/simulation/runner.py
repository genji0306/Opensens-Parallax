...
class ResearchSimulationRunner:
    ...
    def _run_simulation(self, task_id: str, simulation_id: str):
        ...
        try:
            ...
            # Cost optimization: use fast-tier model for agent turns when configured
            ...
            for agent in agents:
                try:
                    ...
                    # If agent has no explicit model and fast tier is configured, use it
                    if not model and fast_model:
                        llm_clients[agent.agent_id] = LLMClient.for_tier("fast")
                    else:
                    ...
                    if default_llm is None:
                        try:
                            default_llm = LLMClient.for_tier("fast") if fast_model else LLMClient()
                        except ValueError:
                            pass
                    if default_llm:
            ...
            # AgentReview-style hard-stop guardrails (belt & braces over the
            # orchestrator's own should_continue). Protects against the
            # inventory "High: infinite loop" risk by ensuring every round
            # has both a round cap and a wall-clock deadline, and by
            # breaking when consecutive rounds show negligible progress.
            simulation_deadline = time.monotonic() + float(
                os.environ.get("PARALLAX_SIM_DEADLINE_S", "1800")
            )
            ...
            for round_num in range(start_round, sim.max_rounds + 1):
                if time.monotonic() > simulation_deadline:
                    logger.warning(
                        "[Runner] simulation %s hit wall-clock deadline at round %d",
                ...
                for agent in agents:
                    ...
                    # Generate response
                    # Cost optimization: cap history to 8 turns (was 20) — later rounds
                    # don't need the full early conversation, saving ~60% input tokens
                    history_window = min(8, len(conversation_history))
                    try:
                        response = agent_llm.chat(
                            ...
                            max_tokens=2000,  # was 4096 default — agent turns rarely exceed 1.5K
                        )
                ...
                if orchestrator and frame and graph_engine and stance_tracker and scoreboard_engine and narrator:
                    ...
                    # Stagnation detection — break if consensus barely moves
                    # across two consecutive rounds AND no new claims land.
                    if last_consensus is not None and abs(consensus - last_consensus) < 0.02 and new_claims == 0:
                    ...
                    if not evaluation.should_continue and round_num < sim.max_rounds:
                        ...
                        break
                    if stagnation_counter >= 2 and round_num < sim.max_rounds:
                        logger.info(
                            "[Runner] stagnation break at round %d (consensus=%.2f)",
                            round_num, consensus,
                        )
                        break
