...
class ResearchReportGenerator:
    ...
    def _get_llm(self, tier: str = "refine") -> Optional[LLMClient]:
        """Get LLM client, preferring cheaper tiered model for report generation."""
        try:
            if LLMClient.model_for_tier(tier):
                return LLMClient.for_tier(tier)
            return LLMClient()
        except ValueError:
            return None

    def _plan_evolution_outline(self, **kwargs) -> Dict[str, Any]:
        llm = self._get_llm(tier="fast")  # Outline planning is structured JSON — use fast tier
