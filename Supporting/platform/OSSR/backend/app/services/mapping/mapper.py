...
class ResearchMapper:
    ...
    def _llm_topic_clustering(self, papers: List[Paper]) -> int:
        ...
        # Use fast keyword fallback — LLM clustering is too slow for pipeline flow
        # (LLM topic clustering can be re-enabled when called outside the pipeline)
        logger.info("Using keyword-based clustering (fast mode) for %d papers", len(papers))
        return self._keyword_clustering_fallback(papers)

    def _llm_topic_clustering_full(self, papers: List[Paper]) -> int:
        """Full LLM clustering — use for standalone mapping, not pipeline."""
        try:
            llm = LLMClient()
        except ValueError:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                result = future.result(timeout=90)  # 90s hard timeout
        except concurrent.futures.TimeoutError:
            logger.warning("LLM clustering timed out after 90s — using keyword fallback")
            return self._keyword_clustering_fallback(papers)
        except Exception as e:
    ...
    def _keyword_clustering_fallback(self, papers: List[Paper]) -> int:
        ...
        for domain_name, group in domain_groups.items():
            try:
                ...
                logger.warning(f"Failed to add domain topic '{domain_name}': {e}")
                continue
            ...
            for sf_name, sf_papers in subfield_groups.items():
                try:
                    ...
                    logger.warning(f"Failed to add subfield topic '{sf_name}': {e}")
                    continue
                ...
                for p in sf_papers:
                    try:
                        ...
                        pass  # Skip FK errors for individual paper-topic links
    ...
    def _analyze_gaps(self) -> int:
        ...
        for i, sf_a in enumerate(subfields):
            for sf_b in subfields[i + 1 :]:
                ...
                # Always use heuristic gap analysis (fast) — LLM gap calls are too slow
