...
class OpenReviewSource(AcademicSourceAdapter):
    ...
    def search(
        ...
        try:
            ...
            if resp.status_code == 429:
                logger.warning("OpenReview rate limited (429), backing off")
                time.sleep(2)
            elif resp.ok:
                data = resp.json()
                ...
                logger.warning(f"OpenReview search returned {resp.status_code}")
        except Exception as e:
        ...
        if len(results) < max_results:
            ...
            for venue in self.VENUES[:4]:  # Limit venue scans
                ...
                try:
                    ...
                    if resp.status_code == 429:
                        logger.warning(f"OpenReview rate limited on venue {venue}")
                        time.sleep(2)
                    elif resp.ok:
                        data = resp.json()
                        ...
                        logger.warning(f"OpenReview venue {venue} returned {resp.status_code}")
                    time.sleep(0.5)  # Rate limit between venue requests
                except Exception as e:
                    logger.warning(f"OpenReview venue scan failed for {venue}: {e}")
...
class IEEEXploreSource(AcademicSourceAdapter):
    """Adapter for IEEE Xplore digital library via their REST API."""

    API_BASE = "https://ieeexploreapi.ieee.org/api/v1/search/articles"

    def __init__(self):
        self.api_key = Config.get("IEEE_API_KEY", "")
        if not self.api_key:
            logger.warning("IEEE_API_KEY not configured — IEEE Xplore adapter disabled")
    ...
    def search(
        ...
        while len(results) < max_results:
            ...
            try:
                ...
                if resp.status_code == 429:
                    logger.warning("IEEE Xplore rate limit hit — stopping")
                    break
                resp.raise_for_status()
                data = resp.json()
            except Exception as exc:
                logger.error("IEEE Xplore API error: %s", exc)
                break
        ...
        logger.info("IEEE Xplore: fetched %d papers for '%s'", len(results), query[:50])
        return results
    ...
    def get_paper(self, identifier: str) -> Optional[PaperMetadata]:
        if not self.api_key:
            return None
        try:
            params = {"apikey": self.api_key, "doi": identifier, "max_records": 1}
            resp = requests.get(self.API_BASE, params=params, timeout=30)
            resp.raise_for_status()
            ...
            logger.error("IEEE Xplore get_paper error: %s", exc)
        return None
    ...
    @staticmethod
    def _parse_article(article: Dict[str, Any]) -> Optional[PaperMetadata]:
        doi = article.get("doi", "")
        ...
        if not doi:
            doi = f"ieee:{article.get('article_number', uuid.uuid4().hex[:12])}"
        ...
        return PaperMetadata(
            ...
            full_text_url=article.get("pdf_url", ""),
            metadata={
...
from .adapters.acm import ACMSource
from .adapters.springer import SpringerSource
from .adapters.core_ac import CORESource
from .adapters.crossref import CrossRefSource
from .adapters.pubmed import PubMedSource
from .adapters.doaj import DOAJSource
from .adapters.europe_pmc import EuropePMCSource
...
SOURCES: Dict[AcademicSource, type] = {
    ...
    AcademicSource.IEEE: IEEEXploreSource,
    AcademicSource.ACM: ACMSource,
    AcademicSource.SPRINGER: SpringerSource,
    # New open-access sources
    AcademicSource.CORE: CORESource,
    AcademicSource.CROSSREF: CrossRefSource,
    AcademicSource.PUBMED: PubMedSource,
    AcademicSource.DOAJ: DOAJSource,
    AcademicSource.EUROPE_PMC: EuropePMCSource,
}
...
class IngestionPipeline:
    ...
    CACHE_TTL_HOURS = {
        ...
        AcademicSource.IEEE: 24 * 7,
        AcademicSource.ACM: 24 * 7,
        AcademicSource.SPRINGER: 24 * 7,
        AcademicSource.CORE: 24,
        AcademicSource.CROSSREF: 24,
        AcademicSource.PUBMED: 24,
        AcademicSource.DOAJ: 24 * 7,
        AcademicSource.EUROPE_PMC: 24,
    }
    ...
    def _get_source_adapter(self, source: AcademicSource) -> AcademicSourceAdapter:
    ...
    def _fetch_single_source(
        ...
        adapter = self._get_source_adapter(source)
    ...
    @staticmethod
    def _merge_metadata(existing: PaperMetadata, incoming: PaperMetadata) -> None:
        """Merge incoming metadata into existing paper, keeping the richest data."""
        ...
        existing.keywords = list(set(existing.keywords) | set(incoming.keywords))
        existing.references = list(set(existing.references) | set(incoming.references))
        if incoming.full_text_url and not existing.full_text_url:
            existing.full_text_url = incoming.full_text_url
        # Keep authors with more affiliation data
        incoming_affs = sum(1 for a in incoming.authors if a.get('affiliation'))
        existing_affs = sum(1 for a in existing.authors if a.get('affiliation'))
        if incoming_affs > existing_affs:
            existing.authors = incoming.authors
        existing.metadata = {**existing.metadata, **incoming.metadata}

    def _parse(self, metadata_list: List[PaperMetadata]) -> List[Paper]:
        """Stage 2: Deduplicate by DOI + fuzzy title matching, normalize, create Paper objects."""
        seen_dois = set()
        title_index: Dict[str, PaperMetadata] = {}
        ...
        for m in metadata_list:
            ...
            # Fuzzy title matching against papers already accepted in this batch
            norm_title = self._normalize_title(m.title)
            if norm_title and len(norm_title) > 10:
                matched_key = None
                for existing_title in title_index:
                    ratio = difflib.SequenceMatcher(None, norm_title, existing_title).ratio()
                    if ratio > 0.85:
                        matched_key = existing_title
                        break
                if matched_key:
                    self._merge_metadata(title_index[matched_key], m)
                    merge_count += 1
                    logger.debug(
                        "Fuzzy-merged paper: '%s' ≈ '%s' (ratio=%.2f)",
                        m.title[:60], title_index[matched_key].title[:60],
                        difflib.SequenceMatcher(None, norm_title, matched_key).ratio(),
                    )
                    continue

            title_index[norm_title] = m
            deduped.append(m)
        ...
        logger.info(
            "Parsed %d new papers (deduped from %d, %d fuzzy-merged)",
            len(papers), len(metadata_list), merge_count,
        )
        return papers
