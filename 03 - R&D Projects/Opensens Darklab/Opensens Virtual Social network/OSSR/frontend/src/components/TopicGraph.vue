...
const zoomLevelName = computed(() => {
  ...
  if (k < 0.5) return 'Domains'
  if (k < 0.8) return 'Subfields'
  if (k < 1.5) return 'Threads'
  return 'Papers'
})
...
const legendItems = computed(() => {
  ...
  if (colorMode.value === 'topic') {
    ...
    return [
      { key: 'domain', label: 'Domain', color: getCSSVar('--graph-domain'), count: domainCount },
      { key: 'subfield', label: 'Subfield', color: getCSSVar('--graph-subfield'), count: subfieldCount },
      { key: 'thread', label: 'Thread', color: getCSSVar('--graph-thread'), count: threadCount },
      { key: 'paper', label: 'Paper', color: getCSSVar('--graph-paper'), count: paperCount }
    ]
  }
