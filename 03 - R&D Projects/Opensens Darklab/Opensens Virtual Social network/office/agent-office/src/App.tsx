...
import { DebateSetupDialog } from "@/components/debate/DebateSetupDialog";
...
const PAGE_MAP: Record<string, PageId> = {
  ...
  "/cron": "cron",
  "/settings": "settings",
  "/debate": "office", // Map debate pages to office for page tracker
};
...
export function App() {
  ...
  const location = useLocation();
  const isDebateRoute = location.pathname.startsWith("/debate");
  ...
  // Debate mode connects directly to OSSR backend — skip Gateway connection gate
  if (isDebateRoute) {
    return (
      <>
        <ThemeSync />
        <Routes>
          <Route path="/debate" element={<DebateSetupDialog />} />
